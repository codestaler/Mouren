...
import PlanesTarjetas from '@/Components/PlanesTarjetas';

const serviciosGenerales = [
    { servicio: "Traslado Inicial", icono: "🚐", cobertura: "Recogida desde el lugar del fallecimiento (clínica o casa) hasta la unidad de preservación." },
    { servicio: "Asesoría Legal", icono: "⚖️", cobertura: "Acompañamiento experto en trámites de defunción, licencias de inhumación y registros notariales." },
    { servicio: "Laboratorio Ético", icono: "🧪", cobertura: "Preparación técnica realizada por profesionales certificados bajo estándares de dignidad absoluta." },
    { servicio: "Cofre y Velación", icono: "🕯️", cobertura: "Suministro de cofre según plan y acceso a salas de velación confortables a nivel nacional." },
    { servicio: "Asistencia 24/7", icono: "📞", cobertura: "Línea de vida Mouren activa siempre para guiar a la familia en cada paso del proceso." },
    { servicio: "Red de Apoyo", icono: "🤝", cobertura: "Convenios en todo el territorio nacional para garantizar el servicio donde se necesite." }
];
...
function AlbumRecuerdos({ onAvanzar, sonando, onToggleTV }) {
    ...
    return (
        <div className="mt-6 w-full flex flex-col items-center">
            <div className="relative bg-[#141210] text-[#FFC600] rounded-xl shadow-2xl p-5 w-[220px] border border-[#FFC600]/30 font-mono">
                {/* esquinas tipo panel/circuito digital */}
                <span className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-[#FFC600] rounded-tl-xl" />
                <span className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-[#FFC600] rounded-br-xl" />
                {/* scanlines sutiles */}
                <div
                    className="absolute inset-0 opacity-10 pointer-events-none rounded-xl overflow-hidden"
                    style={{ backgroundImage: 'repeating-linear-gradient(0deg, #FFC600 0px, transparent 1px, transparent 3px)' }}
                />

                {/* 📺 Botoncito tipo TV: prende/apaga la musiquita de fondo de este plan */}
                <button
                    onClick={onToggleTV}
                    className={`absolute top-2 right-2 z-10 w-7 h-7 rounded-md flex items-center justify-center text-[13px] shadow-md border transition-all hover:scale-110 active:scale-95 ${
                        sonando ? 'bg-[#FFC600] border-[#FFC600] text-[#141210]' : 'bg-[#2a241d] border-[#FFC600]/30 text-[#FFC600]/60'
                    }`}
                    title={sonando ? 'Pausar mensaje de voz' : 'Escuchar mensaje de voz'}
                    aria-label="Alternar mensaje de voz del recuerdo"
                >
                    📺
                </button>

                <p className="text-[8px] uppercase tracking-[3px] opacity-60 mb-2 pr-8">
...
function PlanExperience({ planId, onSerenoToggle, onRumbaToggle, onLegadoAvanzar, onHuellaTap, legadoSonando, onToggleLegadoTV }) {
    switch (planId) {
        case 'sereno': return <VelaInteractiva onToggle={onSerenoToggle} />;
        case 'legado': return <AlbumRecuerdos onAvanzar={onLegadoAvanzar} sonando={legadoSonando} onToggleTV={onToggleLegadoTV} />;
...
// Acentos café/dorado para las tarjetas de "Un Compromiso de Corazón" (misma familia de marca)
const ACENTO_SERVICIOS = ['#5D4E3F', '#A68966', '#8C6A4F', '#C9A876', '#FFC600', '#6E5A3E'];

export default function Planes() {
    const [planActivo, setPlanActivo] = useState(null);
    const [serviciosAbiertos, setServiciosAbiertos] = useState([]);

    const alternarServicio = (i) => {
    ...
    // 🎵 Musiquita ambiente MUY leve mientras se ven las especificaciones de
    // Legado Eterno, Descanso Sereno y Huella Eterna (cada plan con su propia pista).
    const legadoAmbienteRef = useRef(null);
    const serenoAudioRef = useRef(null);
    const huellaAudioRef = useRef(null);

    // 🎙️ Legado Eterno tiene además un mensaje de VOZ aparte, que NO suena solo:
    // solo se reproduce cuando tocan el botoncito 📺.
    const legadoVozRef = useRef(null);
    const [legadoVozSonando, setLegadoVozSonando] = useState(false);
    ...
    // 🎵 Prende la pista ambiente del plan que se está viendo (volumen bajo a
    // propósito: es solo ambiente, no protagonista) y apaga las demás.
    useEffect(() => {
        const pistas = [
            { id: 'legado', ref: legadoAmbienteRef },
            { id: 'sereno', ref: serenoAudioRef },
            { id: 'huella', ref: huellaAudioRef },
        ];

        pistas.forEach(({ id, ref }) => {
            const audio = ref.current;
            if (!audio) return;

            if (planActivo?.id === id) {
                audio.currentTime = 0;
                audio.volume = 0.2; // leve, de fondo
        ...
        // La voz de Legado nunca suena sola: si se sale de ese plan, se detiene.
        if (planActivo?.id !== 'legado') {
            const voz = legadoVozRef.current;
            if (voz) { voz.pause(); voz.currentTime = 0; }
            setLegadoVozSonando(false);
        }
    }, [planActivo]);

    // 📺 El botoncito tipo TV de Legado Eterno prende/apaga SOLO el mensaje de
    // voz (la musiquita ambiente sigue sonando aparte, sin que este botón la toque).
    const alternarVozLegado = () => {
        const voz = legadoVozRef.current;
        if (!voz) return;
        if (voz.paused) {
            voz.currentTime = 0; // es un mensaje hablado, siempre arranca desde el inicio
            voz.volume = 0.9; // clara, no de fondo — aquí sí se quiere entender lo que dice
            const promesa = voz.play();
            if (promesa?.catch) promesa.catch(() => {});
            setLegadoVozSonando(true);
        } else {
            voz.pause();
            setLegadoVozSonando(false);
        }
    ...
    return (
        <div className="min-h-screen bg-[#F4EDE6] font-['Hepta_Slab'] relative overflow-x-hidden flex flex-col text-[#5D4E3F]">
            ...
            {/* --- NUEVA SECCIÓN: TEXTO Y TARJETAS INTERACTIVAS DE SERVICIOS --- */}
            <section className="py-14 sm:py-20 px-4 sm:px-6 bg-white/40">
                <div className="max-w-6xl mx-auto">
                    <div className="mb-10 sm:mb-12 text-center">
                        <span className="text-[#A68966] text-[10px] font-black uppercase tracking-[3px]">Lo que incluye cada plan</span>
                        <h2 className="text-xl sm:text-2xl font-bold text-[#5D4E3F] mt-2 mb-3">Un Compromiso de Corazón</h2>
                        ...
                        <p className="text-[#A68966] text-[10px] font-bold uppercase tracking-wider mt-3">👆 Toca cada tarjeta para ver el detalle</p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                        {serviciosGenerales.map((item, i) => {
                            const abierto = serviciosAbiertos.includes(i);
                            const acento = ACENTO_SERVICIOS[i % ACENTO_SERVICIOS.length];
                            return (
                                <button
                                    key={i}
                                    type="button"
                                    onClick={() => alternarServicio(i)}
                                    aria-expanded={abierto}
                                    className="text-left bg-white rounded-2xl border border-[#5D4E3F]/10 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 overflow-hidden"
                                >
                                    <div className="h-1 w-full transition-colors" style={{ backgroundColor: acento }} />
                                    <div className="p-4 sm:p-5 flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between gap-2">
                                                <h4 className="font-black text-sm text-[#5D4E3F] italic">{item.servicio}</h4>
                                                <span
                                                    className="shrink-0 text-[10px] transition-transform duration-300"
                                                    style={{ color: acento, transform: abierto ? 'rotate(180deg)' : 'rotate(0deg)' }}
                                                >
                                                    ▼
                                                </span>
                                            </div>
                                            ...
                                            >
                                                <p className="overflow-hidden text-[11px] text-[#5D4E3F]/70 leading-relaxed pt-2">
                                                    {item.cobertura}
                                                </p>
                                            </div>
                                        </div>
            ...
            {/* --- SECCIÓN 2: TARJETAS DE PLANES (componente aparte: PlanesTarjetas.jsx) --- */}
            ...
            {planActivo && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-[#F4EDE6]/90 backdrop-blur-md animate-fade-in">
                    ...
                    {/* 🎵 Musiquita de fondo, leve, por plan (Legado / Sereno / Huella) */}
                    <audio ref={legadoAmbienteRef} loop preload="auto">
                        <source src="/audio/planes/legado_ambiente.mp3" type="audio/mpeg" />
                    </audio>
                    <audio ref={serenoAudioRef} loop preload="auto">
                        <source src="/images/planes/audios/Moving.mp3" type="audio/mpeg" />
                    </audio>
                    <audio ref={huellaAudioRef} loop preload="auto">
                        <source src="/audio/planes/huella_ambiente.mp3" type="audio/mpeg" />
                    </audio>

                    {/* 🎙️ Mensaje de VOZ de Legado Eterno: no suena solo, solo con el botón 📺 */}
                    <audio ref={legadoVozRef} preload="auto" onEnded={() => setLegadoVozSonando(false)}>
                        <source src="/images/planes/audios/mouri_msj1.mp3" type="audio/mpeg" />
                    </audio>
                        ...
                        <div className={`md:w-1/3 shrink-0 ${claseFondoPanel(planActivo.id)} p-5 sm:p-6 md:p-8 flex flex-col items-center justify-center text-center overflow-y-auto`}>
                            ...
                            {EXPERIENCIA_PRIMERO.includes(planActivo.id) && (
                                <PlanExperience
                                    ...
                                    legadoSonando={legadoVozSonando}
                                    onToggleLegadoTV={alternarVozLegado}
                                />
                            )}
