...
// 🆕 Notificamos a los administradores de la nueva inscripción
$plan = Plan::find($request->plan_id);
$nombreTitular = $request->afiliados[0]['nombre'] ?? 'Un usuario';

NotificacionService::avisarAdmins(
    'Nueva inscripción a un plan',
    "{$nombreTitular} se inscribió al plan " . ($plan->nombre ?? 'sin nombre') . ".",
    'suscripcion',
    '/admin/ventas'
);

// 🆕 Correo de bienvenida a la suscripción, con firma institucional
$usuarioSuscrito = \App\Models\User::find($userId);
if ($usuarioSuscrito && $usuarioSuscrito->email) {
    $logoUrl = asset('images/logo.png');
    $urlPanel = route('mi.plan');
    $cuotaFormateada = '$' . number_format($suscripcion->cuota_mensual, 0, ',', '.');
    $nombrePlanCorreo = $plan->nombre ?? 'tu plan';
    ...
    <div style='font-family: Georgia, \"Times New Roman\", serif; background:#F4EDE6; padding:32px 16px; margin:0;'>
      <div style='max-width:520px;margin:0 auto;background:#FFFFFF;border-radius:24px;overflow:hidden;box-shadow:0 4px 18px rgba(0,0,0,0.08);'>
        <div style='background:#5D4E3F;padding:28px 24px;text-align:center;'>
          ...
          <p style='color:#F4EDE6;font-size:11px;letter-spacing:2px;text-transform:uppercase;margin:0;opacity:0.8;'>Previsión exequial con calma y respeto</p>
        </div>
        <div style='padding:32px 28px;color:#5D4E3F;'>
          <h1 style='font-size:20px;margin:0 0 8px;'>¡Bienvenido(a) a Mouren, {$nombreTitular}!</h1>
          <p style='font-size:14px;line-height:1.6;color:#6A5A48;margin:0 0 20px;'>
            Tu inscripción se realizó con éxito. A partir de hoy, tu familia cuenta con nuestro acompañamiento y protección.
          </p>
          ...
          <p style='font-size:14px;line-height:1.6;color:#6A5A48;margin:0 0 24px;'>
            Puedes ingresar a tu panel en cualquier momento para agregar beneficiarios, elegir tu música de tributo, tu objeto de memoria y personalizar cada detalle.
          </p>
